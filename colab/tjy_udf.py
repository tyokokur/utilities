...
def git_sync(GIT_REPO, REPO_SUB):
  ...
  exec('!git clone --quiet "{GIT_PATH}" ./temp')      # clone github repository to temp folder
  exec('!mv ./temp/"{REPO_SUB}"/*  ./')
  exec('!rm -rf ./temp')                      # remove all the files/folders in temp folder
  return